// model3.js — Tesla Model 3 Configurator
'use strict';
const MODEL_ID = 'model3';

const VARIANTS = [
  { id:'rwd',  name:'Premium Propulsión Trasera', range:'513', accel:'6.1', speed:'201', price:114990000,
    img:'https://digitalassets.tesla.com/tesla-contents/image/upload/f_auto,q_auto/Homepage-Model-3-Desktop-LHD.jpg' },
  { id:'awd',  name:'Premium Tracción Total',     range:'629', accel:'4.4', speed:'201', price:139990000,
    img:'https://digitalassets.tesla.com/tesla-contents/image/upload/f_auto,q_auto/Homepage-Model-3-Desktop-LHD.jpg' },
  { id:'perf', name:'Performance Tracción Total',  range:'528', accel:'3.1', speed:'262', price:164990000,
    img:'https://digitalassets.tesla.com/tesla-contents/image/upload/f_auto,q_auto/Homepage-Model-3-Desktop-LHD.jpg' },
];
const COLORS = [
...
const WHEELS = [
  { id:'photon', name:'Rines Photon de 18"',
    desc:'Llantas para todas las estaciones<br>Autonomía WLTP: 513 km',
    price:0, dark:true },
  { id:'nova',   name:'Rines Nova de 19"',
    desc:'Llantas para todas las estaciones<br>Autonomía certificada (WLTP): 495 km',
    price:6000000, dark:false },
];
const INTERIORS = [
    ...
    if (!user) {
      ...
      ct.innerHTML = `
        ...
        ${res ? `<div style="background:#f7f7f7;border-radius:8px;padding:12px;border-left:3px solid #3e6ae1;margin-bottom:14px;">
          ...
          <p style="font-size:14px;font-weight:700;margin-top:6px;">${Store.formatPrice(res.totalPrice)}</p>
        </div>
...
document.getElementById('reserve-btn').addEventListener('click', () => {
  ...
  document.getElementById('order-summary').innerHTML =
    `<strong>Model 3 — ${v.name}</strong><br>${COLORS[S.c].name} · ${WHEELS[S.w].name}<br><strong style="font-size:15px;">${Store.formatPrice(getTotal())}</strong>`;
...
document.getElementById('form-payment').addEventListener('submit', async e => {
  ...
  if (payMethod === 'card') {
    ...
    if (num.length < 16) { err.textContent='Número de tarjeta inválido.'; return; }
    if (document.getElementById('card-exp').value.length < 5) { err.textContent='Fecha de vencimiento inválida.'; return; }
    if (document.getElementById('card-cvc').value.length < 3) { err.textContent='CVC inválido.'; return; }
    if (!document.getElementById('card-name').value.trim()) { err.textContent='Ingresa el nombre en la tarjeta.'; return; }
  ...
  Store.saveReservation(contactData.email, {
    modelId:      MODEL_ID,
    variantName:  v.name,
    colorName:    COLORS[S.c].name,
    colorHex:     COLORS[S.c].hex,
    wheelName:    WHEELS[S.w].name,
    interiorName: INTERIORS[S.i].name,
    autopilot:    AUTOPILOTS[S.a].name,
    totalPrice:   getTotal(),
